...
let clickCount=0, threeBV=0;
let moveLog=[];

// UI State
let replaying=false, replayStartTime=0;
let flagMode=false, mobileMode=false, showStats=true;
let leaderboardCategory = "time"; // default category for leaderboard display
...
function init(customMines = null){
  ...
  startTime=null; endTime=null; clickCount=0; threeBV=0; moveLog=[]; replaying=false;
...
statsDiv.addEventListener("click",()=>{ 
  if(bestScores[currentDifficulty]?.[0]) replay(bestScores[currentDifficulty][0]); 
});
...
function saveHighScore(){
  ...
  let entry = { 
    time: elapsed, clickCount, threeBV, threeBVs: threeBVsVal, efficiency: eff, 
    log: moveLog.slice(), win, mines:Array.from(minesSet)
  };

  bestScores[currentDifficulty].push(entry);

  // Compute top 50 per category (for display only)
  let categories=["time","threeBV","threeBVs","efficiency"];
  entry.top50={};
  for(let cat of categories){
    let arr=bestScores[currentDifficulty].slice();
    if(cat==="time") arr.sort((a,b)=>a.time-b.time);
    else arr.sort((a,b)=>b[cat]-a[cat]);
    entry.top50[cat]=arr.slice(0,50);
  }
...
function updateLeaderboard(){
  ...
  let list = bestScores[currentDifficulty];
  if(!list || list.length===0) return;

  // Use top50 of selected category if available
  list = list[list.length-1].top50?.[leaderboardCategory] || list;
...
function replay(entry){
  if(replaying) return;
  replaying=true; firstClick=false; gameOver=false; win=entry.win;
...
function draw(){
  ...
  for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++){
    ...
    if(revealed[r][c]){
      ...
      if(grid[r][c]===-1){ ctx.fillStyle="black"; ctx.beginPath(); ctx.arc(x+TILE/2,y+TILE/2,10,0,Math.PI*2); ctx.fill(); }
    }else{
      ctx.fillStyle="#aaa"; ctx.fillRect(x,y,TILE,TILE); ctx.strokeRect(x,y,TILE,TILE);
      if(flagged[r][c]){ ctx.fillStyle="red"; ctx.beginPath(); ctx.arc(x+TILE/2,y+TILE/2,8,0,Math.PI*2); ctx.fill(); }
    }
    if(gameOver && grid[r][c]===-1 && !revealed[r][c]){ ctx.fillStyle="#f00"; ctx.fillRect(x,y,TILE,TILE); ctx.strokeRect(x,y,TILE,TILE); }
  }
}
